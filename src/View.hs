...
drawPoints :: [(Int,Int)] -> (Int,Int) -> (Int,Int) -> [[Char]]
drawPoints ps (xi,xf) (yi,yf) = drawOn (shiftPoints ps (-xi) (-yi)) (generateGrid (xf-xi+1) (yf-yi+1))


shiftPoints :: [(Int,Int)] -> Int -> Int -> [(Int,Int)]
shiftPoints ps x y = map (\(px, py) -> (px+x,py+y)) ps
...
autogenerateDimensions points = let (xs,ys) = unzip points in
                                (maximum xs - minimum xs + 1,
                                 maximum ys - minimum ys + 1)
